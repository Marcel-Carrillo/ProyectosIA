# Spec: Customer Welcome Email

## Purpose

Automatically reward and re-engage new buyers by issuing a single-use welcome discount coupon and sending a branded welcome email whenever a new `CustomerAccount` is created (local registration or first-time OAuth sign-up). The dispatch is fire-and-forget so SMTP outages never affect the registration response.
