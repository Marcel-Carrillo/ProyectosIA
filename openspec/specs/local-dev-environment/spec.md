# Spec: Local Dev Environment

## Purpose

Defines the local development environment for the women's fashion ecommerce platform: a fully self-contained Docker Compose stack running Postgres 15, Mailpit (email capture), and the Express backend, with no AWS service dependencies. Covers Prisma migration automation, backend hot-reload, email capture, rate-limit configuration across environments, and an optional frontend container profile.
