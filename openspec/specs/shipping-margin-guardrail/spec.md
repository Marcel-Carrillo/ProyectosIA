# Spec: Shipping Margin Guardrail

## Purpose

Admin-only pricing safeguard that surfaces, per variant, whether the public price still covers supplier cost, an estimated freight cost, and a store-configured target margin. Shipping to the customer remains free in every checkout path; this capability never alters checkout totals. Margin figures, freight estimates, and the underlying automation/pricing settings are strictly admin-facing and are never exposed on `/api/public/*` routes, consistent with the existing supplier-cost isolation rule in `product-variant-management`.
