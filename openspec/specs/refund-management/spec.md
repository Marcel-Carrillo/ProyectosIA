...
The system SHALL expose `POST /api/admin/refunds` accepting `customerOrderId` (required), `amount` (required, > 0), `returnRequestId` (optional, nullable), `reason` (optional, max 500 chars), and `paymentProviderReference` (optional, max 150 chars). The system SHALL validate: (1) the order exists; (2) the order payment status is `Paid` or `PartiallyRefunded`; (3) amount does not exceed refundable balance, evaluated inside a Prisma transaction. The new refund SHALL be created with `status = Pending`.
