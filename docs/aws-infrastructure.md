...
| `CJ_SYNC_MAX_PAGES` | Páginas máximas por sync (fallback `'5'`). **Obligatorio en producción, no solo cortesía de rate-limit**: `syncCatalog` hace una llamada `fetchVariants` por producto en cada página, así que el default sin acotar (500×100) nunca termina dentro del timeout de 900s del Lambda — ver incidente en `reports/2026-07-09-production-incident-lock-transaction-revert.md` del change `cj-catalog-auto-provisioning` |
| `CJ_CATALOG_PAGE_SIZE` | Tamaño de página por sync (fallback `'20'`). Ver nota anterior — compartido con el sync manual del panel admin |
