...
| `CJ_SYNC_MAX_PAGES` | Páginas por *ventana* de sync (fallback `'3'`) — cada corrida avanza desde el cursor persistido (`SupplierIntegration.catalogSyncCursorPage`), no reinicia en página 1. **Obligatorio en producción, no solo cortesía de rate-limit**: `syncCatalog` hace una llamada `fetchVariants` por producto en cada página, así que una ventana sin acotar nunca termina dentro del timeout de 900s del Lambda — ver incidente en `reports/2026-07-09-production-incident-lock-transaction-revert.md` del change `cj-catalog-auto-provisioning` |
| `CJ_CATALOG_PAGE_SIZE` | Tamaño de página por sync (fallback `'100'`). Con `CJ_SYNC_MAX_PAGES=3` → ~300 productos/corrida, ~442s estimados (ver estudio de throughput en `openspec/changes/cj-catalog-cursor-and-media/design.md` D2). Compartido con el sync manual del panel admin |
