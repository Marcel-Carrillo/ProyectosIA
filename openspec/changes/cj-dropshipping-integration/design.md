...
1. Add Prisma migration: rename `SpocketCatalogItem` → `CjCatalogItem` (with CJ-specific field additions), change `SupplierIntegration.provider` default, add new `SupplierOrder` external-order fields. **Correction (found during adversarial review, 2026-07-08): the generated migration is actually `DROP TABLE "SpocketCatalogItem"` followed by `CREATE TABLE "CjCatalogItem"` — a destructive drop, not a real rename.** This was safe *only* because the dev database's `SpocketCatalogItem`/`SupplierIntegration` rows were confirmed placeholder/test data and empty at the moment this migration was written and applied. This does **not** generalize to any other environment (e.g. a staging/prod database where the prior Spocket increment had already been exercised) — applying this exact migration file there would silently destroy real data. Any future environment must have its `SpocketCatalogItem` row count verified as zero before this migration is applied, or the migration must be rewritten as a genuine `ALTER TABLE ... RENAME` first.
