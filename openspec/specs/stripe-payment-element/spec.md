# Spec: Stripe Payment Element

## Purpose

Migration of the storefront checkout payment step from the legacy Stripe `CardElement` to the unified `PaymentElement`, enabling cards, Google Pay, and PayPal (via Stripe) to be presented and confirmed through a single component and a single `stripe.confirmPayment` call. No backend PaymentIntent, webhook, or payment-status changes are required for this migration.
