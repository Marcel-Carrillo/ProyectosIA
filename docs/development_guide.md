...
| `CJ_DEFAULT_MARKUP_MULTIPLIER` | Default markup applied to `(CjCatalogItem.supplierCost + CJ_DEFAULT_SHIPPING_ESTIMATE)` when an admin promotes an item (or the scheduled auto-provisioning job promotes one) without an explicit `publicPrice`. Result is rounded to a psychological `,99` ending via `roundToPsychologicalPrice`. `1.6` = 60% margin on cost+shipping. Promotion never publishes at raw cost — if this is unset and no explicit price is given, promotion fails with `CJ_PROMOTION_PRICE_REQUIRED` | No, but required in practice for bulk promotion without per-item prices |
| `CJ_DEFAULT_SHIPPING_ESTIMATE` | Flat shipping cost (store currency) added to `supplierCost` before markup, for every item, when no explicit `publicPrice` is given. A flat value by design, not a live CJ freight quote — CJ's freight API is rate-limited to ~1 req/s, and calling it per item inside `promote()` would risk timing out the Lambda for multi-item batches (the separate `estimateFreight()` endpoint/UI preview still uses a live quote for display purposes) | No — defaults to `0` (no shipping added) if unset |
| `CJ_DEFAULT_CATEGORY_ID` | `Category.id` used as the **fallback** category — by both the manual `POST /cj/catalog/promote` endpoint and the scheduled `supplierAutoProvision` job (see below) — whenever automatic CJ-taxonomy category resolution doesn't produce a category (the item has no CJ category id, the CJ categories API is unavailable, or the resolved id can't be mapped/created). No longer the value always applied: promotion first tries to resolve each item's real category from CJ's own taxonomy (auto-creating a local, initially-`Inactive` `Category` on first use — see `docs/data-model.md`'s `SupplierCategoryMapping`), and only falls back to this env var when that resolution isn't possible. Must reference an **existing** `Category` row | No — but promotion/auto-promotion fails (`CJ_PROMOTION_CATEGORY_REQUIRED`, logged for the scheduled job) for any item where CJ resolution *also* failed and this is unset/invalid |
...
**Backfilling variant size/color for already-synced or promoted CJ items**: `backend/scripts/backfillCjVariantAttributes.ts` re-derives `CjCatalogItem.size`/`color` and linked `ProductVariant.size`/`color` from stored `rawPayload.variant.variantKey` (and fallbacks). Zero CJ API calls; idempotent. Run after deploying the extraction fix to repair rows synced before the fix:
...
**Scheduled auto-provisioning job** (`backend/src/jobs/supplierAutoProvisionHandler.ts`): runs once every 24 hours in production (AWS EventBridge, `rate(1 day)` — see `backend/serverless.yml`'s `supplierAutoProvision` function) and automates the manual connect → verify → sync → promote flow above end to end for any configured supplier provider (CJ Dropshipping today). It auto-creates the `Supplier`/`SupplierIntegration` the first time it detects a configured API key with none provisioned yet, then re-syncs and auto-promotes on every run. Auto-promoted products are always created **Draft** (never auto-activated) using the existing markup-based pricing. Category assignment uses the same CJ-taxonomy resolution as the manual promote endpoint (auto-creating/reusing a local `Category` from each item's own CJ category) — `CJ_DEFAULT_CATEGORY_ID` is only used as a fallback when that resolution fails for an item, not applied unconditionally. An admin still reviews price/category and clicks Activate manually. It has no HTTP route by design; to run it manually (support/debugging or local testing), invoke `handler()` from `backend/src/jobs/supplierAutoProvisionHandler.ts` directly (e.g. via a one-off `ts-node` script loading `dotenv/config`) with `SUPPLIER_AUTO_PROVISION_ENABLED=true`. One-time setup recommended before enabling in any environment: create a fallback `Category` (e.g. "Uncategorized") via the existing admin Category management flow and set its id as `CJ_DEFAULT_CATEGORY_ID`, so promotion never fails outright just because a given item's CJ category couldn't be resolved.

**Backfilling category for already-promoted legacy products**: `POST /api/admin/suppliers/:supplierId/cj/recategorize` (admin-only, rate-limited) re-categorizes products still sitting under the `CJ_DEFAULT_CATEGORY_ID` fallback from before CJ-taxonomy resolution existed (or from a run where resolution failed at the time). Idempotent and safe to re-run — it only touches products still exactly at the fallback category, and never overrides a category an admin has since chosen manually. See `docs/api-spec.yml` for the request/response shape.
