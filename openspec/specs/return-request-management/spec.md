# Spec: Return Request Management

## Purpose

Admin surface for managing return requests raised against customer order items. Enables store administrators to list, create, inspect, and advance return requests through a state machine lifecycle. Return requests are admin-only and serve as the precursor to issuing a linked refund. Supplier-internal data must never appear in return request responses.
