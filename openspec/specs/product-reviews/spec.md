...
Defines verified-buyer product reviews: only customers with a paid purchase of a product's variant may submit a 1–5 star rating (with optional title/body), every new review requires admin moderation before publication, and only `Approved` reviews are ever surfaced — in public read endpoints, rating summaries, or storefront structured data. Reviewer identity is exposed publicly only as a display-name snapshot; the reviewer's email is never exposed.
