...
| `CJ_SYNC_MAX_PAGES` / `CJ_CATALOG_PAGE_SIZE` | Cap catalog sync pages/page-size. Default to 500/100 in this local `.env` file (unset) — for local curl/manual verification against the live API, set both small (e.g. `1`/`5`) to avoid burning rate-limit quota. **Production sets these to `5`/`20`** (via `serverless.yml`, SSM-backed) as a hard requirement, not just a rate-limit courtesy: `syncCatalog` makes one `fetchVariants` API call *per product on every page*, so the unbounded 500×100 default cannot complete within the Lambda's 900s timeout — see `reports/2026-07-09-production-incident-lock-transaction-revert.md` in the `cj-catalog-auto-provisioning` OpenSpec change for the incident that surfaced this | No |
