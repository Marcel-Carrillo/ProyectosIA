...
| `CJ_DEFAULT_MARKUP_MULTIPLIER` | Markup applied to `CjCatalogItem.supplierCost` only when promoting without an explicit `publicPrice`. Formula: `publicPrice = roundToPsychological(supplierCost * markup + CJ_DEFAULT_SHIPPING_ESTIMATE)`. `1.6` = 60% margin on cost; shipping is a flat add-on (not marked up). Promotion never publishes at raw cost — if this is unset and no explicit price is given, promotion fails with `CJ_PROMOTION_PRICE_REQUIRED` | No, but required in practice for bulk promotion without per-item prices |
| `CJ_DEFAULT_SHIPPING_ESTIMATE` | Flat shipping cost (store currency) **added after** markup on cost, for every item, when no explicit `publicPrice` is given. A flat value by design, not a live CJ freight quote — CJ's freight API is rate-limited to ~1 req/s, and calling it per item inside `promote()` would risk timing out the Lambda for multi-item batches | No — defaults to `0` (no shipping added) if unset |
