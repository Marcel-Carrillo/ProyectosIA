# Spec: Pending Order Payment Actions

## Purpose

Customer-facing self-service actions for a `CustomerOrder` stuck in `PendingPayment` status — for example when checkout was abandoned, a card declined, or a wallet redirect (Google Pay/PayPal) was cancelled. Lets the order's owning authenticated customer either resume payment (obtaining a fresh Stripe `clientSecret`, reusing or reissuing the PaymentIntent as needed) or cancel the order outright, and drives the customer-facing UI to show these actions only while payment or cancellation is still possible. Restricted to authenticated customers acting on their own orders; guest orders are out of scope. The `payment_intent.succeeded` webhook remains the single source of truth for marking an order `Paid`, and cancellation re-verifies state immediately before committing to protect against a race with that webhook.
