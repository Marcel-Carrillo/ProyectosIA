...
- [ ] 13.2 Verify all tasks are `[x]` and reports exist under `openspec/changes/cookie-consent-banner/reports/`
- [ ] 13.3 Stage all relevant files (exclude `.env`, `node_modules`, `dist`, `coverage`)
- [ ] 13.4 Create commit: `feat(storefront): add GDPR-compliant cookie consent banner and preferences modal`
- [ ] 13.5 Push branch `feature/cookie-consent-banner` to remote origin
- [ ] 13.6 Create Pull Request with `gh pr create` and report the PR URL in chat
