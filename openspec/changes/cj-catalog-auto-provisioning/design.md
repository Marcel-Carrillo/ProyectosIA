...
- **[Risk]** Two invocations overlapping (a delayed EventBridge retry, or a manual future trigger) could interleave sync/promote calls for the same supplier. → **Mitigation**: use a Postgres advisory lock (`pg_try_advisory_lock`/`pg_advisory_unlock`) keyed on the provider, acquired at the start of that provider's pipeline and released at the end; skip (not block) if already held. **Implementation note (found during adversarial review, before merge):** advisory locks are session-scoped — acquire and release must run on the same physical connection, which two independent `prisma.$queryRaw` calls do not guarantee under Prisma's pooled connections. The acquire → critical section → release span is wrapped in a single `prisma.$transaction(...)` (with a large `timeout` to accommodate the pipeline's external API calls) purely to pin one connection for that span; it is a connection-pinning device, not an atomicity boundary — `ensureSupplierProvisioned`/`descriptor.runPipeline`'s actual business writes still go through the shared `prisma` singleton on their own connections, unchanged.
