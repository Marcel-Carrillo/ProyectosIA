...
Defines the per-locale translation storage model for products, the deterministic locale resolution fallback chain, and the backfill script for seeding existing products with English and machine-generated Spanish translations. Supported locales are `en` and `es`.
