# EscuelaJS Product Import

## Purpose

Defines the one-off import command (`npm run import:products`) that seeds the catalog from the external EscuelaJS API into our domain model. The external API is used only at import time, never at request runtime, and the import is idempotent.
