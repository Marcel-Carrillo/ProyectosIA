# Spec: PayPal + Stripe Checkout

## Purpose

PayPal as a checkout payment method routed entirely through Stripe (Path A), rather than a separate native PayPal SDK integration (Path B). No PayPal-specific backend code, webhook, or refund pipeline is introduced; the existing Stripe PaymentIntent, webhook, and refund flows handle PayPal transparently.
