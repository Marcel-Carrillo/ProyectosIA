# Spec: CJ Catalog Media Capture

## Purpose

Capture supplier-provided product and variant images when staged CJ catalog items are promoted into real products, so promoted products arrive with a main image and product image records instead of appearing image-less in the admin panel and storefront. Applies identically to manual promotion (`cj-catalog-promotion`) and automatic promotion (`supplier-catalog-auto-provisioning`), is idempotent on re-promotion, copies only public-safe data, and includes a backfill path for products promoted before image capture existed.
