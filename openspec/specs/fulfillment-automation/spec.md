# Spec: Fulfillment Automation

## Purpose

Optional, feature-flagged automation layer that chains together customer-order payment success, automatic supplier-order generation, automatic CJ Dropshipping push, and scheduled CJ status sync — while keeping every existing manual admin action from `supplier-order-management`, `cj-supplier-order-push`, and `shipment-management` fully available as a fallback. Automation is off by default; when enabled, it never replaces manual control, only reduces the need for it. Failures are never silent: they are logged without secrets or costs and surfaced through an admin-visible alert path.
