...
- **[Risk]** (found live in production, post-deploy, during task 11.7's manual invocation — not caught by the pre-deploy adversarial review) Both `promote()` (`cjCatalogPromotionService.ts`) and `CjCatalogItemRepository.upsertMany()` wrap a sequential loop over hundreds of DB writes in one `prisma.$transaction(async (tx) => {...})` with no explicit `timeout`, defaulting to Prisma's 5000ms interactive-transaction limit. This pre-existing pattern (from `cj-catalog-auto-provisioning`) was only triggered by this change's own D2 throughput fix, which raises the items processed per run from ~100 to ~900+. Both failures were non-progressing (no partial/corrupt writes — Prisma rolls back the whole transaction), so no data-corruption risk, but every real Lambda invocation failed until fixed. → **Mitigation (fixed, not just documented)**: added `timeout: 120_000` to both transactions (`PROMOTE_TRANSACTION_OPTIONS` / `UPSERT_MANY_TRANSACTION_OPTIONS`), safe because both are pure DB-only work (no external API calls inside the transaction) — unlike the reverted advisory-lock transaction, which spanned external CJ calls inside a Lambda that can freeze mid-transaction. See `reports/2026-07-09-upsertMany-transaction-timeout-fix.md` for the second occurrence's full detail. **Lesson for future changes**: any interactive `prisma.$transaction` wrapping a loop whose size scales with a tunable batch/page-size env var should get an explicit timeout up front, not only after a production failure surfaces it.
