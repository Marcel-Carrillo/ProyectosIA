...
- [ ] 6.1 Load and apply `ai-specs/skills/commit/SKILL.md`
- [ ] 6.2 Verify all tasks above are marked `[x]` and reports exist under `openspec/changes/customer-order-management-backend/reports/`
- [ ] 6.3 Stage relevant files (OpenSpec artifacts, reports, any gap fixes); exclude `.env`, `node_modules/`, `dist/`, `coverage/`
- [ ] 6.4 Create commit: `chore(KAN-28): verify customer order backend API delivery`
- [ ] 6.5 Push branch `feature/KAN-28-customer-order-management-backend` to origin
- [ ] 6.6 Run `gh pr create` and report PR URL in chat
- [ ] 6.7 Transition Jira KAN-28 to **Finalizado** after PR merge (or document in PR if merge pending)
