# Spec: Spocket Connection Management

## Purpose

Admin-only capability for configuring, inspecting, and verifying a supplier's connection to the Spocket dropshipping provider. A `SupplierIntegration` record links a `Supplier` to Spocket; the Spocket API key itself is never accepted as request input or persisted in a returnable column — it is resolved from environment/SSM configuration (`SPOCKET_API_KEY`) at request time. This is the foundation for future supplier automation (see `spocket-catalog-sync` for the read-only catalog pull); order push and webhooks remain out of scope.
