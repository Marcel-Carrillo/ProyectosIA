...
Admin CRUD surface for managing supplier master records. Enables store administrators to create, list, retrieve, update, and soft-delete suppliers from the admin panel. Required precondition for the supplier-order fulfillment flow (KAN-19). Suppliers are strictly internal — no supplier data is exposed on customer-facing APIs.
