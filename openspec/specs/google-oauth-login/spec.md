...
Storefront authentication via Google OAuth 2.0. Allows a customer to sign in or register using their Google account, linking to an existing `CustomerAccount` by email when applicable, and issuing a customer session identical in shape to local login. New Google registrations receive the same welcome coupon and welcome email as local registration. This capability documents the Google-specific OAuth flow, verification, and frontend entry point in detail; see also the `customer-account-authentication` capability for the general multi-provider OAuth requirement and account-linking rule.
